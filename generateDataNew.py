...
def GenCh(f,val, data_shape1, data_shape2, bg_gray, text_gray, text_position):
    img=Image.new("RGB", (data_shape1,data_shape2),(bg_gray,bg_gray,bg_gray))
    ...
    if bg_gray > text_gray:
        ret,bin = cv2.threshold(A[:,:,0],0,255,cv2.THRESH_BINARY_INV+cv2.THRESH_OTSU)
    else:
        ret,bin = cv2.threshold(A[:,:,0],0,255,cv2.THRESH_BINARY+cv2.THRESH_OTSU)
    #cv2.imshow('A',A)
    #cv2.imshow('bin',bin)
    #print A
    #print
...
def motionBlur(img,val):
    ...
    for i in range(3):
        img[i] = cv2.blur(img[i],(blur_kernel0,blur_kernel1),anchor=anchor)
    return img

def AddNoiseSingleChannel(single):
    diff = (255-single.max())/3 
    #print single.shape[0]
    noise = np.random.normal(0,1+r(6),(single.shape[0],single.shape[1]));
    noise = (noise - noise.min())/(noise.max()-noise.min())
    noise= diff*noise;
    noise= noise.astype(np.uint8)
    noise_ = noise[..., np.newaxis]
    noise = np.concatenate((noise_,noise_,noise_),2)
    #print 'noise=', noise
    dst = single + noise
    return dst
...
class GenText:
    def __init__(self, ch_size=16,imgHeight=16,imgWidth=64,inter=0,bg_gray=-1,text_gray=-1,text_position=-1,offset_left=-1):
    ...
    def draw(self,val,font):
        ...
        text_position = random_scale(0,(self.imgHeight-self.ch_size)/2)
        offset_left = int(np.random.random() * self.ch_size)
        offset = offset_left
        ch_num = len(val)
        imgWidth = min(self.imgWidth,offset+ch_num*self.ch_size)
        img = np.array(Image.new("RGB", (imgWidth, self.imgHeight),(bg_gray,bg_gray,bg_gray)))
        base = offset_left
    ...
    def generate(self,text,font):
        ...
        #旋转
        com = rot(fg,r(90)-45,fg.shape,45, (bg_gray,bg_gray,bg_gray))
        #更换背景图片
        ...
        if com.shape[1] < self.imgWidth:
            tmp = np.zeros((self.imgHeight,self.imgWidth,3),dtype='uint8')
            tmp[:,:,:] = 128
...
def genTextImg(num=10000,text_length=8,img_size=20,font_size=20,font_id=1,inter=1,bg_gray=-1,text_gray=-1,text_position=-1,offset_left=-1):
        #num = args.num
        ...
        df = pd.DataFrame(columns =['index','text','img_shape','img'])
        ...
        # 随机设置font_size
            ...
            for size in font_sizes:
                size = size + inter
                Gs.append(GenText(size,img_size,(size+1)*(text_length),inter))
                tmp = []
            ...
            for step in range(0,text_length,1) :
                ...
                Gs.append(GenText(font_size,img_size,(font_size+1)*(text_length-step),inter))
                tmp = []
                tmp.append(ImageFont.truetype('./data/fonts/more_font/仿宋_GB2312.ttf',font_size))
                tmp.append(ImageFont.truetype('./data/fonts/more_font/华文隶书.TTF',font_size))
                tmp.append(ImageFont.truetype('./data/fonts/more_font/宋体_GB18030+%26+新宋体_GB18030.ttc',font_size))
...
if __name__ == '__main__':
        ...
        #parser.add_argument('--img_size',type=int,dest='img_size',default=0,help='flag for the size of image,default is same as img_size')
        ...
        #parser.add_argument('--inter',type=int,dest='inter',default=1,help='flag for the inter between text, range from 1 to 3, default to be 1')
        ...
        #parser.add_argument('--text_position',type=float,dest='text_position',default=-1,help='flag for the text_position, default means random')
        ...
        df = genTextImg(num,text_length,img_size,font_size,font_id,inter,bg_gray,text_gray,text_position,offset_left)
        end_time = time.time()
        print df.info()
